...
/// Bundle mode indicating which proof types are included
#[derive(Debug, Clone, PartialEq)]
pub enum BundleMode {
    /// RFC 3161 timestamp only
    Rfc3161Only,
    /// Blockchain timestamp only
    BlockchainOnly,
    /// Both RFC 3161 and blockchain timestamps
    Combined,
}
...
impl std::fmt::Display for BundleMode {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            BundleMode::Rfc3161Only => write!(f, "RFC 3161 Only"),
            BundleMode::BlockchainOnly => write!(f, "Blockchain Only"),
            BundleMode::Combined => write!(f, "RFC 3161 + Blockchain (Combined)"),
        }
    }
}
...
pub struct CombinedProofBundle {
    ...
    /// Bitcoin block info
    pub bitcoin_block: Option<u64>,
    /// Bundle mode (which proof types are included)
...
pub fn create_combined_proof_bundle(
    ...
    let mut bundle = CombinedProofBundle {
        ...
        timestamp: None,
        bitcoin_block: None,
        bundle_mode: BundleMode::Combined,
    };
    ...
    // Determine bundle mode based on what proofs are present
    let has_rfc3161 = bundle.rfc3161_tsr.is_some();
    let has_blockchain = bundle.ots_proof.is_some();
    
    bundle.bundle_mode = match (has_rfc3161, has_blockchain) {
        (true, true) => BundleMode::Combined,
        (true, false) => BundleMode::Rfc3161Only,
        (false, true) => BundleMode::BlockchainOnly,
        (false, false) => BundleMode::Combined,
    };
...
pub fn generate_proof_manifest(bundle: &CombinedProofBundle) -> AnyhowResult<String> {
    let legal_notes = match bundle.bundle_mode {
        BundleMode::Rfc3161Only => vec![
            "This bundle provides RFC 3161 timestamp proof of document existence.",
            "RFC 3161 timestamp: Issued by a trusted Timestamp Authority.",
            "Legally recognized under eIDAS (EU), ESIGN Act (US), and similar regulations worldwide.",
            "For legal evidence, preserve all files in this bundle along with chain-of-custody documentation.",
        ],
        BundleMode::BlockchainOnly => vec![
            "This bundle provides Bitcoin blockchain timestamp proof of document existence.",
            "Blockchain timestamp: Anchored in Bitcoin blockchain via OpenTimestamps.",
            "Provides decentralized, immutable proof that cannot be altered or removed.",
            "Increasingly recognized in legal contexts; El Salvador, US courts, and academic institutions accept blockchain evidence.",
            "For verification, preserve the .ots file along with the original document.",
        ],
        BundleMode::Combined => vec![
    ...
    let mut manifest = serde_json::json!({
        ...
        "bundle_mode": bundle.bundle_mode.to_string(),
        "original_file": {
    ...
    // Add RFC 3161 section only if relevant
    if bundle.bundle_mode != BundleMode::BlockchainOnly {
        manifest["rfc3161_timestamp"] = serde_json::json!({
    ...
    // Add blockchain section only if relevant
    if bundle.bundle_mode != BundleMode::Rfc3161Only {
        manifest["blockchain_timestamp"] = serde_json::json!({
